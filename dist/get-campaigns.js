...
async function getCampaignList(customerInstance, customerId, campaignIds) {
    const constraints = [
        {
            key: "campaign.status",
            op: "IN",
    ...
    try {
        ...
        console.error(`Error in getCampaignList for customer ${customerId}:`, {
            errorMessage: typeof error === "object" && error !== null && "message" in error ? error.message : String(error),
            errorCode: typeof error === "object" && error !== null && "code" in error ? error.code : undefined,
            errorDetails: typeof error === "object" && error !== null && "response" in error && error.response?.data ? error.response.data : "No response data",
            stack: typeof error === "object" && error !== null && "stack" in error ? error.stack : undefined,
        });
        throw error;
    }
}
async function getCampaignsCostAverage(customerInstance, customerId, customerTimezone, campaignIds, xDays = 3) {
    const constraints = [
        {
            key: "campaign.status",
            op: "IN",
...
function convertStatusEnumToText(status) {
    const statusValue = typeof status === "string" ? parseInt(status) : status;
    const enumToText = ["UNSPECIFIED", "UNKNOWN", "ENABLED", "PAUSED", "REMOVED"];
    return enumToText[statusValue] ?? "UNSPECIFIED";
}
function divideBy1M(value) {
...
export default async function (user, customerId, customerTimezone, campaignIds = []) {
    ...
    customerInstance.client.oauth2Client.on("tokens", (tokens) => {
